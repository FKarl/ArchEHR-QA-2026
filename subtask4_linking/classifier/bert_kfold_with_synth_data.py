...
augmented with Subtask 2 synthetic data.
"""
...
def load_subtask2_synthetic_cases(path: Optional[Path]) -> List[dict]:
...
def build_subtask2_synthetic_train_rows(
    synthetic_cases: List[dict],
    negative_ratio: float,
    seed: int,
) -> List[PairRow]:
    """
    Convert Subtask 2 synthetic relevance records into Subtask 4 pair rows.
    Mapping:
    - synthetic answer text: first essential sentence (or first supplementary if no essential).
    - positive evidence ids: essential + supplementary.
    - negative evidence ids: not in positive set, downsampled by negative_ratio.
    """
    rng = random.Random(seed)
    rows: List[PairRow] = []

    for idx, item in enumerate(synthetic_cases, start=1):
        syn_id = str(item.get("case_id", idx))
        patient_q = str(item.get("patient_question", "")).strip()
        clinician_q = str(item.get("clinician_question", "")).strip()
        ...
        for rel in item.get("relevance_labels", []):
            sid = str(rel.get("sentence_id", "")).strip()
            tag = str(rel.get("relevance", "")).strip().lower()
            if sid not in sent_map:
                continue
            if tag == "essential":
                essential_ids.append(sid)
            elif tag == "supplementary":
                supplementary_ids.append(sid)

        essential_ids = sorted(set(essential_ids), key=numeric_key)
        supplementary_ids = sorted(set(supplementary_ids), key=numeric_key)
        positive_ids = sorted(
            set(essential_ids + supplementary_ids), key=numeric_key
        )
        if not positive_ids:
            continue

        if essential_ids:
            answer_text = sent_map[essential_ids[0]]
        else:
            answer_text = sent_map[supplementary_ids[0]]
        if not answer_text:
            continue

        all_eids = sorted(sent_map.keys(), key=numeric_key)
        pos_set = set(positive_ids)
        negatives = [eid for eid in all_eids if eid not in pos_set]
        if negative_ratio > 0:
            want = int(max(1, round(len(positive_ids) * negative_ratio)))
            if want < len(negatives):
                negatives = rng.sample(negatives, want)

        case_id = f"syn_{syn_id}"
        answer_id = "1"
...
def mix_real_and_synthetic_rows(
    real_rows: List[PairRow],
    synthetic_pool_rows: List[PairRow],
    synthetic_ratio: float,
    seed: int,
) -> Tuple[List[PairRow], int]:
    rng = random.Random(seed)
    train_rows = list(real_rows)
    target_synth = int(round(max(0.0, synthetic_ratio) * len(real_rows)))
    if target_synth <= 0 or not synthetic_pool_rows:
        rng.shuffle(train_rows)
        return train_rows, 0

    if len(synthetic_pool_rows) >= target_synth:
        synth_rows = rng.sample(synthetic_pool_rows, target_synth)
    else:
        # Keep ratio fixed even if pool is smaller by sampling with replacement.
        synth_rows = [
            synthetic_pool_rows[rng.randrange(len(synthetic_pool_rows))]
            for _ in range(target_synth)
        ]

    train_rows.extend(synth_rows)
    rng.shuffle(train_rows)
    return train_rows, len(synth_rows)
...
def run_single_cv(
    ...
    synthetic_pool_rows = build_subtask2_synthetic_train_rows(
        synthetic_cases=synthetic_cases,
        negative_ratio=run_cfg["negative_ratio"],
        seed=args.seed + args._run_counter + 123,
    )
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(
        ...
        train_rows_real = build_gold_train_rows(
            cases=train_cases,
        ...
        train_rows, synth_used = mix_real_and_synthetic_rows(
            real_rows=train_rows_real,
            synthetic_pool_rows=synthetic_pool_rows,
        ...
        print(
            ...
            f"train_rows={len(train_rows)} (real={len(train_rows_real)} synth={synth_used}) "
    ...
    summary = {
        ...
        "synthetic_pool_size": len(synthetic_pool_rows),
...
def train_best_and_predict_test(
    ...
    print(
        f"[final] full-dev train rows: total={len(train_rows)} "
        f"(real={len(train_rows_real)} synth={synth_used})"
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional JSON object or list of objects overriding sweep configs.",
    )
    parser.add_argument(
        "--output_root",
        type=str,
        default="subtask4_linking/output/paper_bert_kfold_subtask2synth",
    )
...
def main():
    ...
    synthetic_cases = load_subtask2_synthetic_cases(
        Path(args.subtask2_synthetic_path)
    )
    print(
        f"Loaded Subtask2 synthetic cases={len(synthetic_cases)} "
        f"synthetic_ratio={args.synthetic_ratio}"
    )

    sweep_summary = None
    if args.mode in {"all", "sweep"}:
        if args.single_config_json:
            parsed = json.loads(args.single_config_json)
            if isinstance(parsed, list):
                selected_cfgs = parsed
            elif isinstance(parsed, dict):
                selected_cfgs = [parsed]
            else:
                raise ValueError(
                    "--single_config_json must decode to dict or list[dict]."
                )
        else:
